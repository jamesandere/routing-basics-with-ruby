...
describe '#Song' do
    ...
    describe("#==") do
        it("is the same song if it has the same attributes as another song") do
            song = Song.new({:name => "All Falls Down", :album_id => @album.id, :id => nil})
            song.save()
            song2 = Song.new({:name => "All Falls Down", :album_id => @album.id, :id => nil})
            song2.save()
    ...
    describe('#update') do
        it('it updates a song by id') do
            song = Song.new({:name => "All Falls Down", :album_id => @album.id, :id => nil})
            song.save()
            song.update("Jesus Walks", @album.id)
            expect(song.name).to(eq("Jesus Walks"))
        end
    end
    ...
    describe('.find_by_album') do
        it("finds songs for an album") do
            album2 = Album.new({:name => "Graduation", :id => nil})
            album2.save()
            song = Song.new({:name => "All Falls Down", :album_id => @album.id, :id => nil})
            song.save()
            song2 = Song.new({:name => "Last Call", :album_id => album2.id, :id => nil})
            song2.save()
